...
function baseCompile(template, options = {}) {
    ...
    if (options.filename) {
        if (!options.filters && options.miniProgram?.filter) {
            ...
            if (options.isX) {
                options.filters.push(utils_1.FILTER_MODULE_NAME);
            }
        }
    }
    ...
    if (options.filename && options.miniProgram?.emitFile) {
        const { class: clazz, directive, emitFile, event, slot, lazyElement, component, checkPropName, } = options.miniProgram;
