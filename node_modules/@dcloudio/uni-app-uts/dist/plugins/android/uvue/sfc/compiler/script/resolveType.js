...
function fileToScope(ctx, filename, asGlobal = false) {
    ...
    const fs = resolveFS(ctx);
    const source = (0, uni_cli_shared_1.preUVueJs)(fs.readFile(filename) || '', filename);
    ...
    try {
        switch (node.type) {
            ...
            case 'TSMethodSignature':
                return ['Function'];
            case 'TSFunctionType':
                let fnType = scope.source.slice(node.start + scope.offset, node.end + scope.offset);
                if (from === 'defineProps') {
            ...
            case 'TSTupleType':
                ...
                let arrType = scope.source.slice(node.start + scope.offset, node.end + scope.offset);
                if (from === 'defineProps') {
