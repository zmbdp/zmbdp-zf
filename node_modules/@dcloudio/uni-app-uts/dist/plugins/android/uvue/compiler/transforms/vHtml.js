...
const transformVHtml = (node, context) => {
    if (!(0, uni_cli_shared_1.isPlainElementNode)(node)) {
        return;
    }
    // check whether bind v-html
    for (const [index, prop] of node.props.entries()) {
        if ((0, uni_cli_shared_1.isDirectiveNode)(prop) && prop.name === 'html') {
            if (!prop.exp ||
                ((0, uni_cli_shared_1.isSimpleExpressionNode)(prop.exp) && prop.exp.content.trim() === '')) {
