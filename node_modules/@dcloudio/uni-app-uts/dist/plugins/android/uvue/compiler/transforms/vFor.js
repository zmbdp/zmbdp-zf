...
exports.transformFor = (0, transform_1.createStructuralDirectiveTransform)('for', (node, dir, context) => {
    ...
    return processFor(node, dir, context, (forNode) => {
        ...
        const listItemProp = (0, compiler_core_2.findProp)(node, `list-item`, false, true);
        if (listItemProp) {
            // 移除该属性
            node.props = node.props.filter((p) => p !== listItemProp);
            if (context.filename) {
                shareStylesVForKey = createShareStylesVForKey(context.filename + ':' + node.loc.start);
            }
        }
        return () => {
            ...
            if (memo) {
                ...
                if (shareStylesVForKey) {
                    renderExp.arguments.push((0, compiler_core_1.createSimpleExpression)(shareStylesVForKey, true));
                }
            }
            else {
                ...
                if (shareStylesVForKey) {
                    renderExp.arguments.push((0, compiler_core_1.createSimpleExpression)('null'), (0, compiler_core_1.createSimpleExpression)('null'), (0, compiler_core_1.createSimpleExpression)(shareStylesVForKey, true));
                }
            }
        };
    });
...
function createShareStylesVForKey(source) {
